...
def get_default_cache_dir():
    return os.environ.get(
        "MODEL_CACHE_DIR", os.path.join(os.path.realpath(os.path.dirname(__file__)), "model")
    )
...
def parse(prediction):
    """Given a list of (word, box) tuple containing the prediction with
    ...
    """
    vert_sort = sorted(prediction, key=lambda p: p[1][:, 1].min())

    hor_sort = []
    curr_line = []

    curr_height_pos = vert_sort[0][1][:,1].min()
    curr_height_range = (vert_sort[0][1][:,1].max() - vert_sort[0][1][:,1].min()) / 2

    for text, box in prediction:
        if box[:,1].min() <= curr_height_pos + curr_height_range:
            curr_line.append((text, box))
        else:
            hor_sort.append(curr_line)
            curr_line = []
            curr_height_pos = box[:,1].min()
            curr_height_range = (box[:,1].max() - box[:,1].min()) / 2
            curr_line.append((text, box))
    hor_sort.append(curr_line)

    sorted_text = ""
    for line in hor_sort:
        sorted_line = sorted(line, key=lambda p: p[1][:, 0].min())
        for text, _ in sorted_line:
            sorted_text = sorted_text + text + " "
